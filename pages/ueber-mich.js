...
const AboutMePage = () => (
	<Page pageName={pageName}>
		...
		<Paragraph timeout={1400}>{getText(`pages.${pageName}.content.textExperience`)}</Paragraph>
		<MobilePicture src="/images/artworks/mobile/bilingual.jpg" alt="Sprachverliebt" />
		<BigTitle timeout={1500} color="blue">
			{getText(`pages.${pageName}.content.titleLanguages`)}
		</BigTitle>
		<Paragraph timeout={1600}>{getText(`pages.${pageName}.content.textLanguages`)}</Paragraph>
		<MobilePicture src="/images/artworks/mobile/girl-pearl.jpg" alt="Menschlich" />
		<BigTitle timeout={1700} color="orange">
			{getText(`pages.${pageName}.content.titleHuman`)}
		</BigTitle>
		<Paragraph timeout={1800}>{getText(`pages.${pageName}.content.textHuman`)}</Paragraph>
	</Page>
)
